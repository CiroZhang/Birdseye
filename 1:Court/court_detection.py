...
Can start from an external camera prior (e.g. VGGT's corrected intrinsic)
instead of a blind initial guess, and exposes a `loss()` you can add other
terms to later (e.g. a MegaSaM-style flow-consistency term).
"""
...
class court_detection:
    ...
    def _solve_pose_and_focal(self, names, n_restarts=4, extra_logf=None):
        ...
        R,t jointly from known correspondences -- no external camera prior
        needed at all, as long as there are enough points (redundancy is
        what makes this well-conditioned instead of degenerate).

        extra_logf: an optional additional log-focal-length candidate to try
        alongside the physically-motivated grid below (e.g. derived from
        VGGT's own bias-corrected FOV estimate for this specific video).
        Purely additive -- it can only help (selected by the same lowest-cost
        + physical-plausibility rule as every other seed) and never makes an
        already-working fit worse, since the original candidates are always
        still tried too."""
