...
results = json.load(open("/tmp/tnv2_demo/all_results.json"))
remaining = [n for n in gt if n not in results]
...
for name in remaining:
    ...
    try:
        subprocess.run([DETECT_BIN, avi, out_pts], capture_output=True, text=True, timeout=90)
    except subprocess.TimeoutExpired:
        print(f"{name}: DETECT TIMED OUT (>90s), skipping", flush=True)
        results[name] = None
